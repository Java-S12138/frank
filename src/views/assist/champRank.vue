...
import {champDict} from "../../../utils/lolData";
import {toPercent,getLacalDateStr} from "../../utils/assistUtils";
import {champSliceListInterface} from "../../types/champRankInterface"

const tier = ref(200)
const champSliceList = ref<champSliceListInterface[]>([])
const options = [
    {
        label: '王者',
        value: 0
...
const restraintList = ref<string[]>([])
...
onMounted(async () => {
    let rankSetDiv:any = document.querySelector('#app > div.slide-in-right > div:nth-child(1) > div')
    rankSetDiv.style['padding-bottom'] = '6px'
  
    await getChampRankData(tier.value,lane.value,getLacalDateStr())
...
// 获取国服英雄数据排行
const getChampRankData = async (tier:number,lane:string,time:number): Promise<any> => {
    let partUrl = 'https://x1-6833.native.qq.com/x1/6833/1061021&3af49f?championid=666'
    let championdetails:string
...
// 获取英雄反制数据
const getRestraintData = async (champId:string|number,position:string,imgUrl:string,name:string,level:string,win:string,ban:string) => {
    restraintList.value = []
    ...
    for (const restraintListElement of restraintJson) {
        ...
        const label:string = champDict[chapmId].label
        const imgUrl:string= `https://game.gtimg.cn/images/lol/act/img/champion/${champDict[chapmId].alias}.png`
...
// 移动窗口
const handleChangePosition = (pos:{x:number,y:number}) => {
    window.electron.send('move-assistWindow', {
        x: pos.x,
        y: pos.y,
    })
}
...
<style scoped>
@import url(../../assets/css/champRank.css);
</style>