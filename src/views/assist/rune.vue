...
import {
    NCard, NAvatar, NSpace, NTag, NGrid, NGi, NIcon, NBadge, NSkeleton,
     NButton, NPopover, NPopconfirm, useMessage
} from 'naive-ui'
import {ref,onMounted} from "vue";
import {champDict, mapNameFromUrl} from '../../../utils/lolData'
import {ArrowBigRightLine, ArrowBigLeftLine} from '@vicons/tabler'
import {request} from "../../../utils/request"

const currentChamp = ref<string>('')
const currentChampImgUrl = ref('')
const currentChampName = ref('请先选择英雄')
const currentChampAlias = ref('')
let runeDataList: any[] = []
const runeDataListFor = ref<string[]>([])
const pageStart = ref(0)
const pageEnd = ref(0)
const limitCount:number = 1
let count:number = 0
const isAutoRune = ref<string|number>(0)
const loading = ref(true)
const skillsAndItems = ref<any[]>([])
const itemCount = ref(1)

const message = useMessage()

onMounted(() => {
    window.electron.on('current-champ-select', (event: any, data: { champId: string | undefined; mode: any; }) => {
        ...
        if (data.champId != currentChamp.value) {
            ...
            itemCount.value = 1
            count = 0
            isAutoRune.value = window.appConfig.has(`autoRune.${data.champId}`) == true ? 'auto' : 0
        }
        count += 1
...
const getRuneData = async (gameMode:string) => {
    if (limitCount == count) {
        ...
        try {
            ...
            pageStart.value = 0
            pageEnd.value = runeDataList.length > 4 ? 4 : runeDataList.length
            runeDataListFor.value = runeDataList.slice(pageStart.value, pageEnd.value)
            if (window.appConfig.has(`autoRune.${currentChamp.value}`) ) {
                message.success('自动配置符文成功')
                // todo
                // 发送消息到聊天界面
                // if (window.appConfig.get('isRecommend')){
                //     sendMessageToChat(window.appConfig.get('credentials'),
...
// 拖动顶部盒子 改变窗口位置
// const handldDrge = (pos:{x:number,y:number}) => {
...
// 上一页
const pageBack = () => {
    if (pageStart.value != 0 && pageEnd.value != runeDataList.length) {
        pageStart.value = pageStart.value - 4
        pageEnd.value = pageEnd.value - 4
        runeDataListFor.value = runeDataList.slice(pageStart.value, pageEnd.value)
    } else if (pageEnd.value == runeDataList.length) {
...
const pageNext = () => {
    ...
    if (pageEnd.value +4 < dataListLen) {
        pageStart.value = pageEnd.value
        pageEnd.value = pageEnd.value +4
        runeDataListFor.value = runeDataList.slice(pageStart.value, pageEnd.value)
    } else if (pageEnd.value < dataListLen) {
        pageStart.value = pageEnd.value
        pageEnd.value = dataListLen
        runeDataListFor.value = runeDataList.slice(pageStart.value, pageEnd.value)
    } else {
...
const deleteAutoRune = () => {
    if (window.appConfig.has(`autoRune.${currentChamp.value}`)) {
        window.appConfig.delete(`autoRune.${currentChamp.value}`)
        isAutoRune.value = 0
    }
}
</script>
...
<style scoped>
@import url('../../assets/css/rune.css');
</style>