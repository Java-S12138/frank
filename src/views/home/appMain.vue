        ...
        <n-card class="boxShadow pointCard" size="small" >
            ...
            <div v-show="switchButton == 1">
                <n-list  >
                    ...
                    <n-list-item>
                        <n-space justify="space-between">
                            <n-tag type="success" :bordered="false" class="tagWidth" v-if="rankData.length !=0" :round="false" size="large">
                                灵活排位
                            </n-tag>
...
const rankData = ref<string[]>([])
const carryData = ref<string[]>([])
const summonerHonor = ref<string[]>([])
const summonerChampLevel = ref<ChapmLevel[]>([])
...
onMounted(() => {
    getHomeData()
    // 监听是否获取到英雄联盟客户端信息
    window.electron.once('client-starting', () => {
        message.loading(
            '英雄联盟客户端启动中...'
        )
    })
    window.electron.once('init-home', (event:any,data:homeData) => {
