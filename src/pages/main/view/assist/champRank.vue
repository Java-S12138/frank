...
const queryKoreaServe = async (tier:number,lane:string) => {
  ...
  try {
    const url = `https://lol.ps/api/statistics/tierlist.json?region=0&version=71&tier=${tier}&lane=${lane}`
...
const getRestraintData = async (champId:any, position:string, imgUrl:string, level:string, win:string, ban:string) => {
  ...
  const url = `https://lol.ps/api/champ/${champId}/versus.json?region=0&version=71&tier=${tierRes}&lane=${position}`
