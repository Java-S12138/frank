...
const updateHaterInfo = async () => {
    ...
    const tagDetails = isHater
        ? ThumbDown[tagIndex.value]
        : ThumbUp[tagIndex.value];
...
const updatePlayerInfo = async () => {
    const userInfos: UserInfos = JSON.parse(
        JSON.stringify(recordStore.userInfos),
    );
    ...
    if (
        tempHaterIdList[platformId][localSumInfo.summonerId].sumIdList.includes(
            hInfo.sumId,
        )
    ) {
        return true;
    }
    tempHaterIdList[platformId][localSumInfo.summonerId].sumIdList.push(
        hInfo.sumId,
    );
    ...
    <div class="pb-2">
        ...
        <n-input
            ...
            :placeholder="
                '请输入' +
                (isHater ? '拉黑' : '点赞') +
                '原因，如果不想输入内容。自动填充左下角信息，点击标签，顺序切换内容'
            "
            style="height: 375px"
        />
