...
<template>
    <n-scrollbar
        id="image-scroll-container-team"
        style="max-height: 378px; padding-right: 18px"
    >
        <div v-if="isLoad === 0">
            <summoner-match-load />
        </div>
                        ...
                        <div>
                            ...
                            <div class="absolute bottom-0 right-0 flex gap-1">
                                <img
                                    ...
                                    :src="getspellImgUrl(match.spell1Id)"
                                />
                                <img
                                    class="h-4 rounded-sm"
                                    :src="getspellImgUrl(match.spell2Id)"
                                />
                            </div>
                        </div>
                    </n-layout-sider>
        ...
        <n-result
            ...
            status="418"
            title="数据获取失败"
            description="或许 这与英雄联盟服务器有关"
        >
            <template #footer> 页面显示数据已替换为召唤师绝活英雄 </template>
        </n-result>
    </n-scrollbar>
</template>
